...
async function requestLinkedinImage(title, content) {
  const image = await openai.images.generate({ prompt: "Give me a simple linkedin post image based on the following data. I need result image without any characters, but can make viewers understand the post. The Linkedin post title is " + title + " and content is : " + content.substring(0, 500) });
